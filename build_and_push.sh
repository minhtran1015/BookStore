...
if [ -z "$DOCKER_HUB_PREFIX" ]; then
    ...
    echo -e "\033[0;31mExample: DOCKER_HUB_PREFIX=\"gcr.io/lyrical-tooling-475815-i8\"${NC}"
    exit 1
fi
...
for service in "${MAVEN_SERVICES[@]}"; do
    ...
    cd $service
    mvn clean install -DskipTests -Dmaven.test.skip=true -Dmaven.test.compile.skip=true
